...
const QUIZ_DATA = [
    // --- Архитектура и проектирование системы (Aura Engine) ---
    { 
        q: "Почему для разработки офлайн-приложения была выбрана платформа Node.js?", 
        a: [
            "Для запуска кода на C++ без компиляции", 
            "Чтобы перенести логику веб-сервера на локальное устройство пользователя", 
            "Для полного отказа от использования JavaScript", 
            "Из-за встроенной поддержки 3D-графики"
        ], 
        c: 1 
    },
    { 
        q: "Какое ограничение технологии PWA решает использование локального сервера в Aura Engine?", 
        a: [
            "Отсутствие поддержки стандартов HTML5", 
            "Лимиты кэша браузера на хранение тяжелых медиаданных (видеоуроков)", 
            "Невозможность адаптивной стилизации интерфейса", 
            "Медленную скорость отрисовки текстовых блоков"
        ], 
        c: 1 
    },
    { 
        q: "Каким образом организовано хранение данных на локальном уровне системы?", 
        a: [
            "В реляционной базе данных PostgreSQL", 
            "В структурированных текстовых файлах формата JSON", 
            "В бинарных файлах реестра операционной системы", 
            "В оперативной памяти без сохранения на диск"
        ], 
        c: 1 
    },
    { 
        q: "Как работает модуль интеллектуальной поддержки (AI Tutor) при отсутствии интернет-соединения?", 
        a: [
            "Отправляет зашифрованные запросы к API Gemini", 
            "Блокирует работу интерфейса приложения до появления сети", 
            "Использует локальную базу знаний для поиска подсказок по ключевым словам", 
            "Генерирует ответы случайным образом"
        ], 
        c: 2 
    },
    { 
        q: "Как в системе Aura Engine минимизируется риск ручного изменения файла progress.json?", 
        a: [
            "Шифрованием по алгоритму RSA-4096", 
            "Генерацией и сверкой контрольных хэш-сумм при запуске", 
            "Скрытием файла в защищенные системные директории Windows", 
            "Запретом доступа пользователя к файловой системе"
        ], 
        c: 1 
    },
    { 
        q: "Какая формула используется алгоритмом системы для расчета прогресса обучения?", 
        a: [
            "(Количество пройденных ID / Общее количество ID в курсе) * 100", 
            "(Общее количество ID - Пройденные ID) * 10", 
            "Сумма всех набранных баллов XP за тесты", 
            "Среднее время, проведенное пользователем на страницах уроков"
        ], 
        c: 0 
    },
    // --- Программная реализация и стек технологий ---
    { 
        q: "Какой инструмент используется для сборки проекта в единый исполняемый файл (.exe)?", 
        a: [
            "Webpack", 
            "PKG", 
            "Electron Builder", 
    ...
    { 
        q: "В каком формате распространяются упакованные учебные курсы в Aura Engine?", 
        a: [
            ".zip", 
            ".aura", 
            ".node", 
    ...
    { 
        q: "Какое решение применено в server.js для обхода блокировки файлов процессами Windows?", 
        a: [
            "Стандартный вызов fs.unlinkSync()", 
            "Функция safeRemove() с циклической попыткой удаления", 
            "Полный перезапуск локального сервера Express.js", 
            "Использование сторонней утилиты очистки реестра"
        ], 
        c: 1 
    },
    { 
        q: "Какая функция отвечает за отправку локального прогресса в Firestore при восстановлении сети?", 
        a: [
            "uploadData()", 
            "syncOfflineData()", 
    ...
    { 
        q: "За счет чего среднее время перехода между уроками составляет менее 100-150 мс?", 
        a: [
            "Использования технологии многопоточности PHP-FPM", 
            "Архитектуры SPA (Single Page Application) и динамического рендеринга DOM", 
            "Высокой пропускной способности внешних интернет-каналов", 
            "Автоматического сжатия изображений на лету"
        ], 
        c: 1 
    },
    { 
        q: "Какой CSS-фреймворк выбран для быстрой загрузки интерфейса в офлайн-режиме?", 
        a: [
            "Bootstrap 5", 
            "Tailwind CSS", 
            "Materialize", 
